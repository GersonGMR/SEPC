			...
			<div class="container">
				...
				<div class="well form-horizontal">
					<fieldset>
						...
						<!-- Form Name -->
						<legend class="text-middle" align="center">Agregar Proyecto</legend>
							...
							<div class="col-md-4 inputGroupContainer">
								<div class="input-group">
									...
									<select id="usuario" name="usuario" class="form-control">	
											...
											<?php
										$res=mysqli_query($conexion,"select * from usuarios where Id_TipoUsu != 1");
							...
							<div class="col-md-4 inputGroupContainer">
								<div class="input-group">
									...
									<select id="certamen" name="certamen" class="form-control">	
											<option disabled selected value="0">Seleccione una categoría</option>
											<?php
										$res=mysqli_query($conexion,"select * from certamen");
										while($row=mysqli_fetch_array($res))
										{
										?>
											<option value="<?php echo $row["id_certamen"];?>"><?php echo $row["fecha_certamen"];?></option>
	...
	$(document).ready(function(){
		$('#registrarNuevo').click(function(){
			...
			if($('#nombre').val()==""){
				...
				if($('#usuario option:selected').val()==0){
					alertify.error("Debes agregar al jurado");

					if($('#carrera option:selected').val()==0){
						alertify.error("Debes agregar la carrera");

						if($('#certamen option:selected').val()==0){
							alertify.error("Debes agregar el certamen");
							return false;
						}
			...
			cadena= "nombre=" + $('#nombre').val() + 
					...
					$.ajax({
						type:"POST",
						url:"../funciones/addproyectos.php",
						data:cadena,
						success:function(r){
							if(r==1){
